-- LMS Schema Migration (retry without IF NOT EXISTS on enum)
...
CREATE POLICY IF NOT EXISTS "Public can view published or creator can view own courses"
ON public.courses FOR SELECT USING (
  is_published = true OR auth.uid() = creator_id
);

CREATE POLICY IF NOT EXISTS "Users can create their own courses"
ON public.courses FOR INSERT WITH CHECK (auth.uid() = creator_id);

CREATE POLICY IF NOT EXISTS "Creators can update their own courses"
ON public.courses FOR UPDATE USING (auth.uid() = creator_id);

CREATE POLICY IF NOT EXISTS "Creators can delete their own courses"
...
CREATE POLICY IF NOT EXISTS "Access modules via course access"
ON public.course_modules FOR SELECT USING (public.can_access_course(course_id));

CREATE POLICY IF NOT EXISTS "Create modules only by course creator"
ON public.course_modules FOR INSERT WITH CHECK (public.can_edit_course(course_id));

CREATE POLICY IF NOT EXISTS "Update modules only by course creator"
ON public.course_modules FOR UPDATE USING (public.can_edit_course(course_id));

CREATE POLICY IF NOT EXISTS "Delete modules only by course creator"
...
-- Allow select if lesson is published and course is accessible, or creator editing
CREATE POLICY IF NOT EXISTS "Select lessons when published or creator"
...
CREATE POLICY IF NOT EXISTS "Creators manage lessons"
...
CREATE POLICY IF NOT EXISTS "Creators update lessons"
ON public.lessons FOR UPDATE USING (
  public.can_edit_course((SELECT m.course_id FROM public.course_modules m WHERE m.id = lessons.module_id))
);

CREATE POLICY IF NOT EXISTS "Creators delete lessons"
...
CREATE POLICY IF NOT EXISTS "Users can view their enrollments"
ON public.enrollments FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Creators can view enrollments for their courses"
ON public.enrollments FOR SELECT USING (
  public.can_edit_course(course_id)
);

CREATE POLICY IF NOT EXISTS "Users can delete their enrollment"
...
CREATE POLICY IF NOT EXISTS "Owner can manage lesson progress"
...
CREATE POLICY IF NOT EXISTS "Owner can insert lesson progress"
...
CREATE POLICY IF NOT EXISTS "Owner can update lesson progress"
...
-- 8) Notifications when lesson is published
...
-- 9) Notify when module completed by a learner
